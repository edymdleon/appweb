...
$(window).on('load', function() {
	...
	var $modal = $('#editor-modal'),
		...
		ft = FooTable.init('#footable-addrow', {
			columns: $.get('https://localhost:44353/Flujo/columnas'),//https://fooplugins.github.io/FooTable/docs/content/columns.json'),
			rows: $.get('https://localhost:44353/Flujo/filas'),//https://fooplugins.github.io/FooTable/docs/content/rows.json'),
			editing: {
				addRow: function(){
				...
				editRow: function(row){
					var values = row.val();
					console.log("idFlujo " + values.idFlujo);
					$editor.find('#idFlujo').val(values.idFlujo);
					$editor.find('#descripcion').val(values.descripcion);
					$editor.find('#estadoRegistro').val(values.estadoRegistro);
					//$editor.find('#status').val(values.status);
					//$editor.find('#dob').val(values.dob.format('YYYY-MM-DD'));
					$modal.data('row', row);
					$editorTitle.text('Edit row #' + values.idFlujo);
					$modal.modal('show');
				},
